				...
				<section id="primary" class="page-with-sidebar with-left-sidebar">
					<div class="content">
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>Log soubor "debug" najdeme ve "var/log/" . Samozřejmně si můžeme zvolit název CCcam logu dle svého uvážení například místo souboru "debug" použijeme "cccamlog.log . Potom spustíme CCcam s následujícím nastavením a parametry:</p>
							<p>
								<code>
									CCcam -d >>/var/log/cccamlog.log &
								</code>
							</p>
							<p>CCcam ukládá log každých 4096 bitů a zatím nevím zdali má nějakou omezenou velikost, takže POZOR na rychlý nárůst velikosti logovacího souboru.</p>
						</div>

						<div class="dt-sc-hr-invisible-very-small"></div>
						<div class="dt-sc-tabs-vertical-container">
							<h5>CCcam log - způsob testu</h5>
							<p>Pro test (test byl určen k popisu CCcam logu verze 2.3.0) jsem server CCcam připojoval na druhý server OSCam s IP 192.168.1.100 a povolenými následujícími porty:
							...
							<p>CCcam server byl nainstalován na DM7025 s příjmem satelitních programů providera pouze freeSAT (CAID 0D02) a bez interních karet.</p>
							<br>
										...
										<div class="portfolio-thumb">
											<figure>
												...
												<div class="image-overlay">
													<a class="zoom" href="images/cccam/graficke_pripojeni_CCcamu_01.png" data-gal="prettyPhoto[gallery]"><span class="fa fa-search"></span></a>
...
21:14:20.142 CCcam: start EMM
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>Přihlášeného uživatele vydíme také ve webovém rozhraní CCcamu na adrese "http://ip_receiver:16001/clients"</p>
						</div>
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>Pokud máme v CCcam.cfg za uživatelem zadanou IP adresu, může se stát že pokud se uživatel připojí z jiné IP, CCcam server ho chápe jako neexistujícího</p>
...
F: user1 pass1 192.168.1.55
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>V případě, že použijeme CCcam server jako klienta s připojením na další CCcam server, pro správnou ECM instrukci pro DVB API se zobrazí následující v logu</p>
							<div class="dt-sc-one">
								<pre class="brush: text;">
...
00:13:43.941 CCcam: <-[31;1mremote ecm <- 192.168.1.100:27010 ok (took 0.799 seconds)<-[0m
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>V případě, že na CCcam server připojíme uživatele/klienta (řádek v CCcam.cfg - F: user1 pass1) a tento CCcam server připojíme na další CCcam server, pro správnou ECM instrukci klienta se v logu zobrazí následující </p>
							<div class="dt-sc-one">
								<pre class="brush: text;">
...
16:07:09.890 CCcam: configfile changed. Updating...
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>Připojení na newcamd server (N: 127.0.0.1 10000 dummy dummy 01 02 03 04 05 06 07 08 09 10 11 12 13 14 v CCcam.cfg)</p>
							<div class="dt-sc-one">
								<pre class="brush: text;">
...
16:07:10.474 CCcam: newcamd prov 05 0000a0  00000000
							...
							<p>Z logu je patrné, že se CCcam server připojil na Newcamd server kde na portu "27020" je povolena karta s CAID 0D96.<br>
								Tento log se zobrazí v případě, že je správně nastavené uživatelské jméno a heslo v CCcam.cfg v řádku "N: ....."</p>
						</div>
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>Připojení na newcamd server se špatným uživatelským jménem (N: 127.0.0.1 10000 dummy dummy 01 02 03 04 05 06 07 08 09 10 11 12 13 14 v CCcam.cfg)</p>
							<div class="dt-sc-one">
								<pre class="brush: text;">
...
16:25:14.853 CCcam: newcamd receive error: no login ack
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>Připojení na newcamd server se špatným uživatelským heslem (N: 127.0.0.1 10000 dummy dummy 01 02 03 04 05 06 07 08 09 10 11 12 13 14 v CCcam.cfg)</p>
							<div class="dt-sc-one">
								<pre class="brush: text;">
...
16:25:14.853 CCcam: newcamd receive error: no login ack
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<h4><strong class="article_number">15. </strong> CCcam log - připojení na newcamd server (ECM OK pro DVB API)</h4>
							<p>Po připojení na newcamd server se správným nastavením (N: 127.0.0.1 10000 dummy dummy 01 02 03 04 05 06 07 08 09 10 11 12 13 14 v CCcam.cfg)
							   začne CCcam DVB API dekódovat a v případě, že CCcam server dostane správné ECM odpovědi od Newcamd serveru se zobrazí následující</p>
							<div class="dt-sc-one">
								<pre class="brush: text;">
...
22:39:23.807 CCcam: .[31;1mnewcamd ecm <- 192.168.1.100:27019 ok (took 0.565 seconds).[0m
						...
						<div class="dt-sc-tabs-vertical-container">
							...
							<p>Pro zobrazení časů v sekundách musíme v CCcam.cfg mít položku "SHOW TIMING : yes". SHOW TIMING je defaultně vypnuto.</p>
...
22:39:23.807 CCcam: .[31;1mnewcamd ecm <- 192.168.1.100:27019 ok (took 0.565 seconds).[0m
							...
							<p><span class="fa icon-icon_light"></span><strong>Zobrazeno časování s vypnutím zobrazením časování v CCcam.cfg "SHOW TIMING : no"</strong></p>
							<p>I přesto, že nemáme v CCcam.cfg zadanou položku "SHOW TIMING : yes" můžeme spustit CCcam přes telnet se zapnutím časování</p>
							<code>
								CCcam -t
							</code>
							<p>Více informací o spouštění CCcamu přes telnet najdeme v sekci <a href="index.php?category=cccam&page=cccam-telnet">Telnet</a></p>
						</div>
...
14:08:27.347 CCcam: cam[0] ecm odd nok caid:0x653 id:0x0 pid:0x183f Unknown   
						...
						<div class="dt-sc-tabs-vertical-container">
							<div class="intro-text type2">
								...
								<ul>
									<li><a href="http://www.csservis.com/knowledgebase/2/Cccam-English-Tutorial.html" target="_blank">Cccam-English-Tutorial</a></li>
									<li><a href="http://www.zebradem.com/wiki/index.php?title=CCcam_Fehlermeldungen" target="_blank">ZebraDem - CCcam chybová hlášení</a></li>
								</ul>
								<p style="position: absolute; bottom: 20px; right: 20px;">
									<i>Vyčerpávající popis CCcam logu (skyndas)</i> <i class="fa fa-smile-o" style="font-size:25px;font-weight:bold;margin-left:10px"></i>
								</p>
							</div>
						</div>
