      ...
      <div class="dt-sc-tabs-vertical-frame-content">
        <div class="clearfloat">
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>don't clear SMARTCARD SID ASSIGN and LOADBALANCE settings when configfile changes (changing these
              settings requires a restart, so they should not be touched when config is re-read)
            </li>
            ...
            <li>added GLOBAL EMM setting (clientside), to control generation of GA EMM traffic (see CCcam.cfg for
              details)
            </li>
          </ul>
          ...
          <ul class="dt-sc-fancy-list arrow">
            <li>improved system selection (based on distance). Note that prios from prio file overrule this and might
              give worse results
            </li>
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>moved providernames to an external file /var/etc/CCcam.providers. Format is 'hexvalue "providername"
              comments'
            </li>
            <li>channelname can now be displayed in webif instead of caid:ident:sid when defined in
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>added optional ip address/hostname to F: line, no ip given allows the connection from all ip's (See
              CCcam.cfg for example)
            </li>
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>added timeslots per friend connection, now possible to only open connection for a few hours (See
              CCcam.cfg for example)
            </li>
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>added column in client info output to see the ecm time per client, only available if SHOW TIMING is on
            </li>
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>userinfo was not correctly reread, therefore you needed to restart CCcam when certain parts of the
              userinfo changed, fixed
            </li>
            <li>it is now possible to block a channel for a client, based on caid:id:sid see F entry config for an
              example
            </li>
            <li>changed emu share limits behaviour, &lt; 1.3.0 an emu matched a F entry limit with {caid:ident:n}, for
              any nonzero ident. >= 1.3.0, an emu only matches {caid:0:n}
            </li>
            <li>ports 15000-15004 and port 14000 are now gone. Use 16000 and 16001 (html) instead</li>
            <li>implemented workerthread pool to handle client requests, to improve handling larger numbers of clients
            </li>
            <li>added support for cryptoworks camcrypted cards (incl arena tv). IPK handing is internal, no need for
              external keys and such
            </li>
            <li>added support for BEEF patched cards using nagra over seca tunnelling, add id's in config file(see
              example)
            </li>
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>web info interface on port 16001
              (As of now getting info from ports 15000-15004 is deprecated. This interface will be removed in the next
              release.)
            </li>
            <li>newcamd stealth mode added</li>
            <li>removed 1.1.1 backward compatibility code from share protocol, to allow full use of optimisations
              introduced in 1.1.1
            </li>
            <li>when CCcam was connecting to a share it kept on connecting on same ip, not very good when a dyndns
              address was in the config
            </li>
            ...
            <li><strong>WARNING:</strong> F: entry syntax changed, check and update your CCcam.cfg; added parameters to
              allow emusharing and emm, per client
            </li>
          </ul>
          ...
          <ul class="dt-sc-fancy-list arrow">
            <li>when a share doesn't work for a certain channel, skip aliases of that share for that channel
              (aliases are the same cards shared through different routes, they have equal nodeID_shareID at the end of
              their line in the output on port 15002)
            </li>
            ...
            <li>optimized share protocol (backward compatibility with &lt; 1.1.1 guaranteed, warning, will be removed in
              next release)
            </li>
          </ul>
          ...
          <ul class="dt-sc-fancy-list arrow">
            ...
            <li>when a card is reachable through multiple routes, each unique route will be available as individual
              share
              ******* with nodeid 'abc' in the routelist are never sent to client with nodeid 'abc', to avoid route
              loops
            </li>
